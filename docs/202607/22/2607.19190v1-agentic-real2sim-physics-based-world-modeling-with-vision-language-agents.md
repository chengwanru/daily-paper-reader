...
title_zh: 基于代理的实到仿：基于视觉语言智能体的物理世界建模
...
score: 7.0
evidence: 基于视觉语言代理的物理世界建模，从真实录制生成可模拟仿真
tldr: 真实到模拟转换仍需大量人工操作。Agentic Real2Sim框架利用视觉-语言智能体，自动从机器人交互记录恢复几何、状态和物理参数，生成可运行仿真。在刚体、柔体和人形运动场景中验证，使用开源VLM即可达到与前沿模型相当的成功率。该工作为机器人策略学习和评估提供了可扩展的转换途径。
source: arxiv
...
motivation: 真实到模拟转换依赖手工视觉模型调参和流程胶合，导致效率低下。
method: 以视觉-语言智能体为核心，自动恢复几何、状态和物理参数，组装成可运行仿真。
result: 在刚体、柔体和人形运动场景中成功转换，使用开源VLM即达到与前沿模型相当的成功率。
conclusion: 为机器人策略学习与评估提供真实对齐的可仿真孪生。
---

## 摘要
机器人对物体的交互进行实到仿转换仍需要大量人工，因为它不仅需要视觉重建：一个简化的实到仿过程必须恢复场景几何和物体状态，推断物理参数，并将执行器、物体、相机、位姿和轨迹组装成可运行的物理仿真。如今，这一过程仍依赖于手动调整视觉基础模型、网格清理、坐标帧对齐，以及在视觉感知工具和仿真器之间脆弱的流程胶水。我们提出基于代理的实到仿，一种基于视觉语言智能体的通用物理世界建模框架，将物体-机器人交互的真实世界记录转换为可仿真的情节双胞胎，该双胞胎保留了观测、几何、机器人交互和物体状态。我们在刚体操作、可变形物体交互和人形运动场景上评估了基于代理的实到仿，涵盖通常由单独实到仿流水线处理的领域，标志着向可扩展转换迈出了第一步。该框架的代理决策可由一个开放权重的VLM后端驱动，成本仅为前沿模型的一小部分，同时达到相当的转换成功率。我们旨在将生成的真实对齐双胞胎用于下游机器人任务，特别是策略学习和评估。项目网站见https://ericchen321.github.io/agentic_real2sim.github.io/。
